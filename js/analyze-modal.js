...
const AnalyzeModal = (() => {
    ...
    function openConfig(opts) {
        ...
        if (ds)
            ds.value = isoDateForInput(sevenDaysAgo);
        const de = $('#acf-date-end');
        if (de)
            de.value = isoDateForInput(now);
        // Reset focus
        const focus = $('#acf-focus-input');
        if (focus)
            focus.value = '';
        ...
        if (q) {
            entries = entries.filter(([_, s]) => {
                ...
                if ((s.aliases || []).some(a => (a || '').toLowerCase().includes(q)))
                    return true;
                if ((s.twitter_handle || '').toLowerCase().includes(q))
                    return true;
                return false;
            });
        }
    ...
    function readConfig() {
        ...
        for (const key of Object.keys(SUBJECT_REGISTRY)) {
            ...
            if (s.display_name.toLowerCase() === name.toLowerCase()) {
                subject = s;
                break;
            }
        }
        if (!subject) {
            for (const key of Object.keys(SUBJECT_REGISTRY)) {
                const s = SUBJECT_REGISTRY[key];
                if ((s.aliases || []).some(a => a.toLowerCase() === name.toLowerCase())) {
                    subject = s;
                    break;
                }
            }
    ...
    function openDashboard(report) {
        ...
        if (name)
            name.textContent = report.subject.display_name;
        if (handle)
            handle.textContent = report.subject.twitter_handle ? '@' + report.subject.twitter_handle : 'No Twitter handle';
        ...
        if (biasEl)
            biasEl.textContent = report.bias.bias;
        ...
        if (status)
            status.textContent = 'Generated ' + new Date(report.generatedAt).toLocaleTimeString();
    ...
    function setMetric(key, value, meta) {
        ...
        if (card) {
            ...
            if (key === 'hypocrisy') {
                ...
                if (value < 25)
                    bucket = 'sd-metric-low';
                else if (value > 60)
                    bucket = 'sd-metric-high';
            }
            else {
                if (value >= 70)
                    bucket = 'sd-metric-high';
    ...
    function renderJustification(report) {
        ...
        for (const t of (report.tweets || [])) {
            ...
            if (s > 1)
                hypPos.push({ text: t.text, score: s });
            else if (s < -1)
                hypNeg.push({ text: t.text, score: s });
        }
        ...
        try {
            ...
            if (config.source_scope === 'twitter' || config.source_scope === 'both') {
                if (!config.subject.twitter_handle) {
                    ...
                    if (config.date_end)
                        opts.endTime = toIsoDate(config.date_end, true);
                    if (clamped) {
