 ...
 */
const AI = (() => {
    const STOPWORDS = new Set(('a an and or the of in to for on with at by from as is are was were be been ' +
        ...
        'new said says say tell told now today yesterday tomorrow amid amid per via').split(/\s+/));
    ...
    function computeTrendingInfo(articles, fullCorpus) {
        if (!Array.isArray(articles) || !Array.isArray(fullCorpus))
            return;
        ...
        for (const a of articles)
            if (a && a._tx && typeof a._tx.importance === 'number')
                scored++;
