...
(async () => {
  ...
  function showReel() {
    ...
    if (typeof window.PublishModal !== 'undefined' && window.PublishModal.setCurrentUser) {
      ...
      function resetPullIndicator() {
        ...
        if (pullLabel) pullLabel.textContent = 'Pull to refresh';
      }
      ...
      container.addEventListener('touchmove', e => {
        ...
        if (currentReelIndex < 3 && dy > 0 && (!isSwiping || isPulling) && Math.abs(dy) > Math.abs(dx) && dy > 10) {
          ...
          if (pullLabel) pullLabel.textContent = dy >= REFRESH_THRESHOLD ? 'Release to refresh' : 'Pull to refresh';
        }
      ...
      container.addEventListener('touchend', e => {
        ...
        if (wasPulling && !pullTriggered && dy >= REFRESH_THRESHOLD) {
          pullTriggered = true;
          if (pullLabel) pullLabel.textContent = 'Refreshing...';
          if (pullIndicator) pullIndicator.classList.add('releasing');
      ...
      if (isQuote) {
        ...
        if (quoteImageRemove) {
          quoteImageRemove.onclick = () => {
            ...
            if (quoteImagePreview) quoteImagePreview.style.display = 'none';
          };
        }
  ...
  function drawDualShadowText(ctx, text, x, y, opts: { align?: string; maxWidth?: number } = {}) {
  ...
  function drawDualShadowTextTight(ctx, text, x, y, opts: { align?: string } = {}) {
  ...
  async function init() {
    ...
    try { syncViewToggleBtn(); } catch (e) { /* noop */ }
    ...
    if (footerRow) footerRow.style.display = '';
    try { updateStickyHeader(); } catch (e) { /* noop */ }
    hideLoadingOverlay();
