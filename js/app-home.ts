    ...
    if (googleBtn) {
      googleBtn.addEventListener('click', () => {
        if (window.location.protocol === 'file:') {
          const msg = $('#auth-msg');
          if (msg) { msg.textContent = 'Google sign-in requires a web server (http:// or https://).'; msg.style.color = 'var(--error, #e03131)'; }
          return;
        }
