...
const TwitterFetcher = (() => {
    ...
    function clearCache(handle) {
        try {
            localStorage.removeItem(CACHE_PREFIX + handle);
        }
        catch { }
    }
    ...
    async function fetchTweets(handle, options) {
        if (!handle)
            return { ok: false, reason: 'no-handle', message: 'No Twitter handle provided' };
        options = options || {};
        ...
        if (result.ok && Array.isArray(result.tweets))
            writeCache(handle, result.tweets);
        return result;
    }
    return {
        fetchTweets,
