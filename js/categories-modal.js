...
window.CategoriesModal = (() => {
    ...
    function renderTabs() {
        ...
        $$('.cat-pane').forEach(pane => {
            const isActive = (pane.id === 'cat-pane-news' && activeTab === 'news') ||
    ...
    function renderNewsPane() {
        ...
        const parts = [];
        parts.push('<button class="cat-pill cat-pill-all' + (allActive ? ' active' : '') +
        ...
        for (const s of subs) {
            ...
            parts.push('<button class="cat-pill' + (isActive ? ' active' : '') +
    ...
    function renderInternationalMenu() {
        ...
        return '<div class="cat-intl-menu">' +
            cards.map(c => '<button class="cat-intl-card" data-intl-list="' + escapeHtml(c.id) + '" type="button">' +
    ...
    function renderInternationalList(key) {
        ...
        if (!list.length)
            return '<p class="cat-empty">No feeds in this list.</p>';
        const byCountry = {};
        ...
        return '<button class="cat-back-btn" id="cat-back-btn" type="button">' +
            ...
            '</button>' +
            countries.map(c => '<section class="cat-parliament-section">' +
    ...
    function renderParliamentGrid(items) {
        if (!items.length)
            return '<p class="cat-empty">No items.</p>';
