...
const ScoringEngine = (() => {
  ...
  function sentenceSentiment(sentence) {
    ...
    for (const tok of tokens) {
      if (NEGATORS.has(tok)) { negate = !negate; continue; }
  ...
  function corpusSentiment(text) {
    ...
    let total = 0;
    for (const s of sents) total += sentenceSentiment(s);
  ...
  function buildTfidfVectors(docs) {
    if (!docs.length) return { vectors: [], vocab: new Map() };
    const N = docs.length;
  ...
  function scoreHypocrisy(articles, tweets) {
    if (!tweets || !tweets.length) return { score: 0, opposingPairs: 0, totalPairs: 0, reason: 'no-tweets' };
    const articleText = (articles || []).map(a => (a.title || '') + '. ' + (a.summary || '')).join(' ');
    if (!articleText.trim()) return { score: 0, opposingPairs: 0, totalPairs: 0, reason: 'no-articles' };
    ...
    if (total === 0) return { score: 0, opposingPairs: 0, totalPairs: 0, reason: 'no-pairs' };
  ...
  function scoreRelevance(articles, tweets) {
    if (!tweets || !tweets.length) return { score: 0, reason: 'no-tweets' };
    const tweetText = (tweets || []).map(t => t.text || '').join(' ');
    const articleText = (articles || []).map(a => (a.title || '') + ' ' + (a.summary || '')).join(' ');
    if (!tweetText.trim() || !articleText.trim()) return { score: 0, reason: 'no-text' };
  ...
  function scoreBias(tweets) {
    if (!tweets || !tweets.length) return { bias: 'Center', left: 0, right: 0, reason: 'no-tweets' };
    let left = 0, right = 0;
    for (const t of tweets) {
      const tokens = tokenize(t.text || '');
      for (const tok of tokens) {
        if (LEFT_WORDS.has(tok)) left++;
        else if (RIGHT_WORDS.has(tok)) right++;
      }
    }
    let bias = 'Center';
    if (left === 0 && right === 0) bias = 'Center';
    else if (left > right * 1.5) bias = 'Left';
    else if (right > left * 1.5) bias = 'Right';
  ...
  function scoreFactualClarity(articles, opts) {
    ...
    if (!sourceWeights.length) return { score: 0, sources: [], reason: 'no-sources' };
  ...
  function findPotentialSources(subject, allFeeds) {
    if (!subject || !allFeeds || !Array.isArray(allFeeds)) return [];
    ...
    for (const f of allFeeds) {
      ...
      for (const t of tokenSet) {
        if (hay.indexOf(t) !== -1) { hit = true; break; }
      }
  ...
  function analyze({ subject, articles, tweets, opts }) {
    ...
    if (opts.focusTopic) {
      ...
      if (focusTokens.length) {
        ...
        if (!focusedArticles.length) focusedArticles = articles;
      }
    }
    ...
    for (const t of tweets || []) {
      ...
      for (const tk of toks) {
        ...
        if (SENTIMENT_LEXICON[tk]) continue;
        if (LEFT_WORDS.has(tk) || RIGHT_WORDS.has(tk)) continue;
