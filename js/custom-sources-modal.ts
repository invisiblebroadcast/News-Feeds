...
window.CustomSourcesModal = (() => {
  ...
  function renderNationSelect() {
    ...
    sel.innerHTML = Object.keys(nations).map(k =>
      '<option value="' + k + '"' + (k === current ? ' selected' : '') + '>' + (nations[k] || k) + '</option>'
    ).join('');
  ...
  async function handleValidate() {
    ...
    try {
      ...
      validatedFeed = null;
      if (msg) { msg.textContent = 'Validation failed'; msg.className = 'feed-validate-msg error'; }
    }
  }
  ...
  async function handleAdd() {
    ...
    const msg = $('#cs-validate-msg');
    if (!name || !url) {
      if (msg) { msg.textContent = 'Enter a name and URL first.'; msg.className = 'feed-validate-msg error'; }
      return;
    }
    try { await FeedManager.addCustomFeed(name, url, scope, nation, subcat, 'en'); }
    catch (e) { console.warn('addCustomFeed failed:', e && e.message); }
    validatedFeed = null;
    const urlInput = $('#cs-url-input'); if (urlInput) urlInput.value = '';
    const nameInput = $('#cs-name-input'); if (nameInput) nameInput.value = '';
  ...
  function bindAll() {
    ...
    if (urlInput) urlInput.addEventListener('keydown', e => { if (e.key === 'Enter') handleValidate(); });
    const scopeSel = $('#cs-scope-select');
    if (scopeSel) scopeSel.addEventListener('change', renderNationSelect);
  }
