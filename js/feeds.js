...
const FeedManager = (() => {
    ...
    async function load() {
        ...
        loadPromise = fetch('data/feeds.json')
            .then(r => { if (!r.ok)
            throw new Error('Failed to load feeds'); return r.json(); })
            .then(data => {
            feedData = data;
    ...
    function getSelectedNation() {
        try {
            return localStorage.getItem(SELECTED_NATION_KEY) || defaultNation();
        }
        catch {
    ...
    function getFeeds(scope, nation) {
        ...
        for (const f of allSubs) {
            ...
            if (!f.hasRss || !f.url)
                continue;
            if (f.scope === 'global' && scope === 'global')
                feeds.push({ name: f.name, url: f.url, hint: f.hint || 'politics', lang: f.lang || 'en' });
            if (f.scope === 'nation' && scope === 'nation' && f.nation === nation)
                feeds.push({ name: f.name, url: f.url, hint: f.hint || 'politics', lang: f.lang || 'en' });
        }
        return feeds;
    ...
    function _readLocalCustomFeeds() {
        try {
            return JSON.parse(localStorage.getItem(CUSTOM_FEEDS_KEY) || '[]');
        }
        catch {
            return [];
        }
    ...
    function getSubscribedFeeds() {
        try {
            return JSON.parse(localStorage.getItem('newsfeeds_subscriptions') || '[]');
        }
        catch {
            return [];
        }
    ...
    function getParliamentItemById(id) {
        if (!id || !feedData?.parliamentFeeds)
            return null;
