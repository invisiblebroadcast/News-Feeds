...
const FeedFetcher = (() => {
    ...
    function detectFeedTimezoneOffset(feed) {
        ...
        // India: +5:30 (330 min)
        if (/\.in\b/.test(url) || name.includes('india') || region.includes('india'))
            return 330;
        // UK: +0:00 in winter, +1:00 in summer — skip for now, use 0
        // US East: -5:00 (-300 min)
        if (region.includes('us east') || region.includes('usa'))
            return -300;
        return 0;
    }
    ...
    function parseRssXml(xmlText, feed) {
        ...
        for (const item of items) {
            ...
            const pubDate = correctPubDateTimezone(item.querySelector('pubDate')?.textContent || '', feed);
    ...
    async function proxyFetchPaginated(feed) {
        ...
        for (let page = 2; page <= maxPages + 1; page++) {
            try {
                ...
                if (added === 0)
                    break; // no new items = we've exhausted history
            }
            catch (e) {
    ...
    function afterFetch(feed, items, err) {
        if (!window.SourceHealth || !feed || !feed.url)
            return;
        ...
        if (ok) {
            ...
            if (window.ArticleArchive) {
                for (const a of items)
                    ArticleArchive.ingest(a, feed.lang);
            }
        }
        else {
    ...
    function deduplicate(articles) {
        ...
        return articles.filter(a => {
            ...
            if (titleKey && seen.has('t:' + titleKey))
                return false;
            seen.add(linkKey);
            if (titleKey)
                seen.add('t:' + titleKey);
