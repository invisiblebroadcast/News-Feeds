...
const PublishModal = (() => {
    ...
    async function fetchVideoTitle(videoId) {
        try {
            ...
            if (res.ok) { const data = await res.json(); return data.title || ''; }
        } catch { }
        return '';
    }
    ...
    let _ibPostImageFile = null;
    function initIbPostImageHandlers() {
        const fileInput = $('#ib-post-image');
        const preview = $('#ib-post-image-preview');
        const previewImg = $('#ib-post-image-preview-img');
        const removeBtn = $('#ib-post-image-remove');
        if (fileInput) {
            fileInput.addEventListener('change', () => {
                const file = fileInput.files[0];
                if (!file) return;
                if (file.type !== 'image/png' && file.type !== 'image/jpeg') { fileInput.value = ''; return; }
                if (file.size > 2 * 1024 * 1024) { fileInput.value = ''; return; }
                _ibPostImageFile = file;
                if (preview && previewImg) { previewImg.src = URL.createObjectURL(file); preview.style.display = 'block'; }
            });
        }
        if (removeBtn) {
            removeBtn.addEventListener('click', () => {
                _ibPostImageFile = null;
    ...
    async function handleFetchTranscript() {
        ...
        try {
            const [transcript, videoTitle] = await Promise.all([fetchTranscript(videoId), fetchVideoTitle(videoId)]);
            if (!transcript || !transcript.length) throw new Error('No transcript available for this video');
            if (videoTitle && titleInput) titleInput.value = videoTitle;
            const transcriptText = transcript.map(t => t.text).join('\n');
            if (descInput) descInput.value = transcriptText;
            setMsg(msg, 'Transcript fetched successfully!', 'success');
        } catch (e) { setMsg(msg, e.message || 'Failed to fetch transcript', 'error'); }
        finally { if (fetchBtn) { fetchBtn.disabled = false; fetchBtn.textContent = 'Fetch Transcript'; } }
    }
        ...
        try {
            ...
            const { data: inserted, error } = await client
                ...
                .insert({
                    ...
                    author: currentUser.email || '', title, body: desc,
                    source_name: 'YouTube', source_link: url || '',
                    scope: 'global', nation: '', category: 'all',
                    type: 'feeds', quote_from: ''
                })
                .select('post_id').single();
            if (error) throw error;
            if (_ibPostImageFile && inserted?.post_id) {
                try {
                    const ext = _ibPostImageFile.type === 'image/png' ? 'png' : 'jpg';
                    const filePath = formatPostId(inserted.post_id) + '.' + ext;
                    await client.storage.from('ib-post-images').upload(filePath, _ibPostImageFile, { contentType: _ibPostImageFile.type, upsert: true });
                } catch (imgErr) { console.warn('[PublishModal] IB image upload failed:', imgErr.message); }
            }
            ...
            if ($('#ib-post-image')) $('#ib-post-image').value = '';
            if ($('#ib-post-image-preview')) $('#ib-post-image-preview').style.display = 'none';
        } catch (e) { setMsg(msg, e.message || 'Publish failed', 'error'); }
    ...
    async function handlePublishQuote() {
        ...
        try {
            ...
            const { data: inserted, error } = await client
                ...
                .insert({
                    user_id: currentUser.id, user_email: currentUser.email || '',
                    author: currentUser.email || '', title: '', body: desc,
                    source_name: quoteFrom || '', source_link: sourceLink || '',
                    scope: scopeVal, nation: scopeVal === 'nation' ? 'india' : '',
                    category: 'quotes', type: 'quote', quote_from: quoteFrom || ''
                })
                .select('post_id').single();
            if (error) throw error;
            ...
            if (_quoteImageFile && postId) {
                ...
                const { error: uploadErr } = await client.storage.from('ib-post-images').upload(filePath, _quoteImageFile, { contentType: _quoteImageFile.type, upsert: true });
                if (uploadErr) console.warn('[PublishModal] Image upload failed:', uploadErr.message);
            }
        ...
        finally { if (publishBtn) { publishBtn.disabled = false; publishBtn.textContent = 'Publish Quote'; } }
    }
    ...
    function setCurrentUser(user) { currentUser = user; updateFabVisibility(); }
    function openModal() {
        ...
        if (window.appState && typeof window.appState.openModal === 'function') { window.appState.openModal('publish', modal); }
        else { modal.classList.add('open'); }
        showTab('youtube');
    }
    function closeModal() {
        if (window.appState && typeof window.appState.closeModal === 'function') { window.appState.closeModal('publish'); }
        else { const modal = $('#yt-publish-modal'); if (modal) modal.classList.remove('open'); }
    }
    function bindAll() {
        const fab = $('#publish-fab');
        if (fab) fab.addEventListener('click', openModal);
        $$('.publish-tab').forEach(tab => { tab.addEventListener('click', () => showTab(tab.dataset.publishTab)); });
        const fetchBtn = $('#publish-fetch-btn');
        if (fetchBtn) fetchBtn.addEventListener('click', handleFetchTranscript);
        const publishBtn = $('#yt-publish-btn');
        ...
        if (quotePublishBtn) quotePublishBtn.addEventListener('click', handlePublishQuote);
        initQuoteImageHandlers();
        initIbPostImageHandlers();
    }
    return { bindAll, setCurrentUser, openModal, closeModal };
})();
