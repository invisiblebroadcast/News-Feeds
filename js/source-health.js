...
const SourceHealth = (() => {
    ...
    function emit(evt) { for (const fn of listeners) {
        try {
            fn(evt);
        }
        catch { }
    ...
    function syncDisabledState() {
        ...
        for (const url of Object.keys(map)) {
            ...
            if (!s)
                continue;
            if (on && (s.failures || 0) >= FAILURE_THRESHOLD)
                s.disabled = true;
        }
        save(map);
        emit({ type: 'sync', map });
    ...
    function isDisabled(url) {
        ...
        if (!(Settings && Settings.get && Settings.get('autoDisableFailingSources')))
            return false;
        const map = load();
