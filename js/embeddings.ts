...
const Embeddings = (() => {
  ...
  function needsConsent() {
    if (_userConsented !== null) return false;
    const stored = localStorage.getItem('embeddings_consent');
    if (stored === '1') { _userConsented = true; return false; }
    if (stored === '0') { _userConsented = false; return false; }
    return true;
  }
  ...
  async function loadModelWithProgress(onProgress) {
    if (model) { if (onProgress) onProgress({ status: 'ready', progress: 1 }); return model; }
    if (loadPromise) return loadPromise;
    if (!hasConsent()) return null;

    if (onProgress) onProgress({ status: 'download', phase: 'library', progress: 0 });

    loadPromise = (async () => {
      try {
        if (onProgress) onProgress({ status: 'download', phase: 'library', progress: 0.1 });

        const { pipeline } = await import('https://esm.sh/@huggingface/transformers@3.4.2');

        if (onProgress) onProgress({ status: 'download', phase: 'model', progress: 0.15 });
        ...
        isReady = true;
        if (onProgress) onProgress({ status: 'ready', progress: 1 });
        return model;
      } catch (e) {
        loadError = e;
        if (onProgress) onProgress({ status: 'error', error: e && e.message });
        console.warn('[Embeddings] all-MiniLM-L6-v2 load failed:', e && e.message);
        return null;
      }
    })().finally(() => {
