...
(async () => {
  ...
  function bindSubjectSuggest() {
    ...
    input.addEventListener('input', () => {
      ...
      timer = setTimeout(() => {
        ...
        if (!matches.length) { suggest.innerHTML = ''; suggest.style.display = 'none'; return; }
        suggest.innerHTML = matches.map(s =>
          '<div class="acf-suggest-item" data-subject=\'' + JSON.stringify(s).replace(/'/g, '&#39;') + '\'>' +
  ...
  function bindRunAnalysis() {
    ...
    runBtn.addEventListener('click', async () => {
      ...
      if (!subjectName) {
        if (subjectInput) subjectInput.classList.add('acf-input-error');
        setTimeout(() => { if (subjectInput) subjectInput.classList.remove('acf-input-error'); }, 600);
        return;
      }

      let source = 'both';
      for (const radio of sourceRadios) {
        if (radio.checked) { source = radio.value; break; }
      }
