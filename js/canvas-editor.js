...
(function () {
    ...
    function cleanSummary(s) {
        if (!s)
            return '';
        // Preserve newlines but collapse whitespace within each line
    ...
    function open(article, includeImage, loadedImg, onComplete) {
        ...
        if (imgSection)
            imgSection.style.display = (includeImage && loadedImg) ? '' : 'none';
        ...
        if (bodyColorSection)
            bodyColorSection.style.display = isQuote ? 'none' : '';
        if (advTextSection)
            advTextSection.style.display = isQuote ? 'none' : '';
