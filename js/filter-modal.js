...
const FilterModal = (() => {
    ...
    function openFilterModal() {
        ...
        if (ds)
            ds.value = currentFilter.date_start;
        if (de)
            de.value = currentFilter.date_end;
    ...
    function updateActiveCount() {
        ...
        if (n === 0 && !currentFilter.date_start && !currentFilter.date_end) {
            ...
            if (currentFilter.date_start || currentFilter.date_end)
                parts.push('date');
    ...
    function renderSourceList() {
        ...
        if (hint)
            hint.textContent = '(showing ' + (isIndia ? 'India' : 'Global') + ' sources)';
        ...
        const scoped = allFeeds.filter(f => {
            if (!f || !f.name || !f.hasRss)
                return false;
            if (isIndia)
                return f.scope === 'nation' && (!f.nation || f.nation === 'india');
        ...
        if (currentFilter.sources.size === 0) {
            for (const f of scoped) {
                if (subscribed.has(f.url))
                    currentFilter.sources.add(f.name);
            }
        }
        ...
        list.querySelectorAll('.filter-source-row').forEach(row => {
            row.addEventListener('click', (e) => {
                ...
                if (!name)
                    return;
                if (cb.checked)
                    currentFilter.sources.add(name);
                else
                    currentFilter.sources.delete(name);
                e.preventDefault();
            });
        });
    }
    ...
    function selectAll() {
        list.querySelectorAll('.filter-source-cb').forEach(cb => {
            ...
            if (row)
                currentFilter.sources.add(row.dataset.name);
        });
    }
    function selectNone() {
        list.querySelectorAll('.filter-source-cb').forEach(cb => {
            ...
            const row = cb.closest('.filter-source-row');
            if (row)
                currentFilter.sources.delete(row.dataset.name);
        });
    }
    function bindAll() {
        const open = $('#filter-modal-btn');
        if (open)
            open.addEventListener('click', openFilterModal);
        const close = $('#filter-modal-close');
        if (close)
            close.addEventListener('click', closeFilterModal);
        const cancel = $('#filter-modal-cancel');
        if (cancel)
            cancel.addEventListener('click', closeFilterModal);
        ...
        if (sn)
            sn.addEventListener('click', selectNone);
