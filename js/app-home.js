...
(async () => {
    ...
    function currentFrameId() {
        if (modalStack.length > 0)
            return modalStack[modalStack.length - 1].frameId;
        if (subViewStack.length > 0)
            return subViewStack[subViewStack.length - 1].frameId;
        return -1;
    }
    ...
    function installBackTrap(useReplace) {
        ...
        try {
            if (useReplace)
                history.replaceState({ ibTrap: true }, '');
            else
                history.pushState({ ibTrap: true }, '');
        }
        catch { }
    }
    ...
    function bucketBySharedTitleToken(articles) {
        const STOP = new Set(('the and for are but not you all can her was one our had has his how man new now old see two way who boy did its let put say she too use from with this that have will your what when make like long look many some them then than been call come could does each find first from have like make more only over part people said take than them there these they time used want water which word work would write about after again also around another away back because before').split(/\s+/));
        const map = new Map();
        for (const a of articles) {
            ...
            if (!map.has(key))
                map.set(key, { sources: new Set(), count: 0 });
            const b = map.get(key);
            if (a.source)
        ...
        for (const b of map.values())
            out.push({ sources: b.sources.size, count: b.count });
        return out;
    }
    ...
    function renderExploreView() {
        ...
        const tabsHtml = EXPLORE_TABS.map(t => '<button class="explore-inner-tab' + (t.key === currentExploreTab ? ' active' : '') + '" data-explore-tab="' + t.key + '">' +
    ...
    function renderExploreCategories(container) {
        ...
        container.innerHTML = '<div class="explore-cat-grid">' +
            CATEGORIES.map(c => '<a href="index.html?subcat=' + c.id + scopeParam + '" class="category-card explore-cat-card">' +
    ...
    function openReelsForArticle(link, articles, exitRestore) {
        ...
        try {
            history.pushState({ ibFrame: reelsFrameId, ibReels: true }, '');
        }
        catch { }
        showReel();
    }
    ...
    function updateCard(cardEl, article, idx, total) {
        ...
        if (shareImgBtn)
            shareImgBtn.style.display = hasThumb ? '' : 'none';
                ...
                conflictBody.innerHTML = c.conflicts.map(group => {
                    ...
                    return '<div class="rcp-row">' +
                        ...
                        group.detail.map(g => '<span class="rcp-value">' + escHtml(g.value) + '</span>' +
                            '<span class="rcp-sources">(' + g.articles.map(a => escHtml(a.source || 'Unknown')).join(', ') + ')</span>').join(' vs ') +
                        '</div>';
                }).join('');
        ...
        if (flagEl) {
            if (ad.flag) {
                flagEl.textContent = ad.flag;
                flagEl.style.display = 'inline';
                flagEl.style.background = FLAG_COLORS[ad.flag] || 'var(--text-tertiary)';
            }
            else {
        ...
        if (readBtn)
            readBtn.dataset.article = encodeURIComponent(article.link);
        const st = cardEl.querySelector('.reels-share-text');
        if (st) {
            st.dataset.url = encodeURIComponent(article.link);
            st.dataset.title = article.title;
            st.dataset.source = article.source;
        }
        const si = cardEl.querySelector('.reels-share-image');
        if (si) {
            si.dataset.url = encodeURIComponent(article.link);
            si.dataset.title = article.title;
            si.dataset.source = article.source;
        }
    ...
    function showReel() {
        ...
        if (!existing) {
            ...
            stack.addEventListener('click', e => {
                ...
                if (likeBtn) {
                    ...
                    if (lBtn)
                        lBtn.classList.toggle('active', !!ad.like);
                    if (dBtn)
                        dBtn.classList.toggle('active', !!ad.dislike);
                    return;
                }
                ...
                if (reelPublishBtn) {
                    ...
                    if (!requireAuth())
                        return;
                    if (currentArticle)
                        openPublishModal(currentArticle);
                    return;
                }
                ...
                if (reelEditBtn) {
                    ...
                    if (currentArticle && currentArticle._isPublished && currentArticle._pubId)
                        editPublishedArticle(currentArticle._pubId);
                    return;
                }
                ...
                if (commentBtn) {
                    if (!requireAuth())
                        return;
                    if (currentArticle)
                        openCommentsPage(currentArticle);
                    return;
                }
        ...
        if (dots)
            dots.innerHTML = articles.map((a, i) => '<span class="reels-dot' + (i === idx ? ' active' : '') + '"></span>').join('');
    ...
    function closeReelsReader() {
        ...
        if (scroll) {
            const f = scroll.querySelector('.rr-fetched');
            if (f)
                f.remove();
        }
        if (el.articleModal.classList.contains('open'))
            closeArticleModal();
    }
    ...
    function renderConflictsView(container) {
        ...
        for (const a of all) {
            ...
            if (!clusters.has(key))
                clusters.set(key, { conflicts: c.conflicts, severity: c.severity || 0, articles: [], clusterSize: c.clusterSize || 0 });
        ...
        try {
            ...
            root.querySelectorAll('.topic-card').forEach(card => {
                ...
                card.addEventListener('click', e => {
                    if (e.target.closest('.topic-critical-pill'))
                        return;
    ...
    function renderBuildArticle(article, cluster) {
        ...
        if (title)
            title.textContent = article.headline || 'Build Article';
        ...
        if (copyBtn)
            copyBtn.addEventListener('click', () => copyBuildArticle(article, 'text'));
    ...
    function articleToMarkdown(article) {
        ...
        for (const s of (article.sources || [])) {
            const t = s.title || s.link || 'Untitled';
            if (s.link)
                lines.push('- [' + t + '](' + s.link + ')' + (s.name ? ' — ' + s.name : ''));
            else
                lines.push('- ' + t + (s.name ? ' — ' + s.name : ''));
        }
        return lines.join('\n').trim();
    }
    ...
    function openPublishModal(article, sources) {
        ...
        if (titleInput)
            titleInput.value = article.subject?.display_name || article.title || '';
        if (descInput)
            descInput.value = article.summary || '';
        if (status)
            status.textContent = '';
        if (authorInput)
        ...
        if (sourcesContainer) {
            if (sourceList.length) {
                sourcesContainer.innerHTML = sourceList.map(s => '<label class="publish-source-item">' +
        ...
        if (scopeSelect) {
            ...
            scopeSelect.onchange = () => {
                ...
                if (wrap)
                    wrap.style.display = scopeSelect.value === 'nation' ? '' : 'none';
            };
            scopeSelect.onchange();
        }
        if (nationSelect)
            nationSelect.value = article._pubNation || currentNation || 'india';
        if (categorySelect)
            categorySelect.value = article._pubCategory || article.feedHint || currentSubcat || 'all';
    ...
    function renderPublishedFeed() {
        ...
        if (published.length) {
            ...
            if (container) {
                container.innerHTML = published.slice(0, 5).map(p => '<div class="published-item">' +
    ...
    async function editPublishedArticle(pubId) {
        ...
        try {
            ...
            if (diffDays > 3) {
                alert('Edit window expired (3 days from publishing)');
                return;
            }
            ...
            if (isQuote) {
                ...
                if (postDesc)
                    postDesc.value = data.body || '';
                if (postSourceName)
                    postSourceName.value = data.source_name || '';
                if (postSourceLink)
                    postSourceLink.value = data.source_link || '';
                if (postScope)
                    postScope.value = data.scope || 'global';
                if (postCategory)
                    postCategory.value = data.category || 'all';
                if (postMsg) {
                    postMsg.textContent = '';
                ...
                if (postBtn) {
                    ...
                    postBtn.onclick = async () => {
                        ...
                        if (!t) {
                            _setElMsg(postMsg, 'Please enter a title', 'error');
                            return;
                        }
                        if (!d) {
                            _setElMsg(postMsg, 'Please enter content', 'error');
                            return;
                        }
                        ...
                        try {
                            ...
                            await fetchPublishedArticlesFromSupabase();
                            if (postMsg) {
                                postMsg.innerHTML = '\u2705 Updated!';
                                postMsg.className = 'publish-msg success';
                            }
                            ...
                            if (postMsg) {
                                postMsg.textContent = '\u274c ' + (e.message || 'Update failed');
                                postMsg.className = 'publish-msg error';
                            }
    ...
    async function deletePublishedArticle(pubId) {
        if (!currentUser)
            return;
        if (!confirm('Delete this IB post? This cannot be undone.'))
            return;
        try {
            const client = getSupabaseClient();
            if (!client)
            ...
            const diffDays = (Date.now() - pubDate.getTime()) / (1000 * 60 * 60 * 24);
            if (diffDays > 3) {
                alert('Delete window expired (3 days from publishing)');
                return;
            }
    ...
    function bindFilterToggles() {
        ...
        if (el.filterToggle) {
            el.filterToggle.addEventListener('click', () => {
                ...
                if (el.sortPanel)
                    el.sortPanel.style.display = 'none';
                el.filterToggle.classList.toggle('active', hidden);
                if (el.sortBtn)
                    el.sortBtn.classList.remove('active');
            });
        }
    ...
    function bindTranslate() {
        ...
        if (el.translateModal) {
            ...
            if (closeBtn)
                closeBtn.addEventListener('click', () => closeModal('translate'));
            el.translateModal.addEventListener('click', e => {
                if (e.target === el.translateModal)
                    closeModal('translate');
            });
        }
        ...
        if (clusterModal) {
            ...
            if (closeBtn)
                closeBtn.addEventListener('click', () => closeModal('cluster'));
    ...
    function isGoogleNewsRedirect(url) {
        try {
            return new URL(url).hostname === 'news.google.com' && url.includes('/rss/articles/');
        }
        catch {
            return false;
        }
    ...
    function updateSortOptions() {
        if (!el.sortBy)
            return;
        if (!el.sortBy.value)
            el.sortBy.value = currentSort || 'date-desc';
    ...
    async function renderContent() {
        ...
        try {
            ...
            const groups = {};
            const quickResults = await Promise.allSettled(quickFeeds.map(f => FeedFetcher.fetchFeed(f, QUICK_PER_SOURCE_CAP)));
            let quickCount = 0;
            ...
            if (restFeeds.length > 0) {
                ...
                setTimeout(() => {
                    if (!abortFlag.aborted)
                        backgroundFetchRest(key, feeds, quickFeeds.length, abortFlag);
                }, 250);
            }
        }
        catch (err) {
    ...
    async function backgroundFetchRest(key, allFeeds, startIndex, abortFlag) {
        ...
        for (let i = startIndex; i < allFeeds.length; i += REST_BATCH_SIZE) {
            ...
            const settled = await Promise.allSettled(batch.map(f => FeedFetcher.fetchFeed(f, REST_PER_SOURCE_CAP)));
    ...
    function totalFeedsForKey(key) {
        if (totalFeedsByKey[key])
            return totalFeedsByKey[key];
    ...
    async function displayCurrentSubcat() {
        ...
        if (!skipHeavyWork) {
            ...
            for (const cat of Object.keys(cached.groups)) {
                if (Array.isArray(cached.groups[cat]))
                    fullCorpus.push(...cached.groups[cat]);
            }
    ...
    async function applySemanticRank(articles) {
        if (!currentSearch || !articles || !articles.length)
            return;
        ...
        try {
            ...
            for (let i = 0; i < articles.length; i++) {
                ...
                if (!articleEmbedding && text) {
                    ...
                    if (articleEmbedding)
                        Embeddings.setCached(a.link, articleEmbedding);
                }
                if (myVersion !== _searchVersion)
                    return;
    ...
    function bindSourcesConfig() {
        if (el.sourcesConfigModalClose)
            el.sourcesConfigModalClose.addEventListener('click', closeSourcesConfigModal);
        if (el.sourcesConfigDone)
            el.sourcesConfigDone.addEventListener('click', closeSourcesConfigModal);
        if (el.sourcesConfigModal)
            el.sourcesConfigModal.addEventListener('click', e => { if (e.target === el.sourcesConfigModal)
                closeSourcesConfigModal(); });
    ...
    function renderFeedHealth() {
        ...
        if (countEl) {
            if (count === 0) {
                ...
                const parts = [];
                if (disabled.length)
                    parts.push('<strong>' + disabled.length + '</strong> disabled');
                if (failing.length)
                    parts.push('<strong>' + failing.length + '</strong> with ' + (SourceHealth.WARN_AT || 2) + '+ failures');
    ...
    function bindSettings() {
        ...
        el.modal.addEventListener('click', e => { if (e.target === el.modal)
            closeSettings(); });
        if (el.hardRefreshBtn)
            el.hardRefreshBtn.addEventListener('click', openHardRefreshModal);
        ...
        if (el.autoDisableFailingSources) {
            el.autoDisableFailingSources.addEventListener('change', () => {
                ...
                if (SourceHealth)
                    SourceHealth.syncDisabledState();
        ...
        if (el.hardRefreshModalClose)
            el.hardRefreshModalClose.addEventListener('click', closeHardRefreshModal);
        if (el.hardRefreshCancel)
            el.hardRefreshCancel.addEventListener('click', closeHardRefreshModal);
        if (el.hardRefreshConfirm)
            el.hardRefreshConfirm.addEventListener('click', performHardRefresh);
        ...
        if (optionsBtn && headerExtras) {
            ...
            document.addEventListener('click', (e) => {
                if (headerExtras.style.display === 'none')
                    return;
                if (headerExtras.contains(e.target))
                    return;
    ...
    function renderActivityTab(tab) {
        ...
        const articleMap = {};
        if (currentArticles)
            currentArticles.forEach(a => articleMap[a.link] = a);
        container.innerHTML = filtered.map(([link, data]) => {
            ...
            const badges = [];
            if (data.like)
                badges.push('<span class="ai-badge" style="background:var(--accent-dim);color:#fff">&#x1F44D;</span>');
            if (data.dislike)
                badges.push('<span class="ai-badge" style="background:var(--text-tertiary);color:#fff">&#x1F44E;</span>');
            if (data.flag)
                badges.push('<span class="ai-badge" style="background:' + (FLAG_COLORS[data.flag] || 'var(--text-tertiary)') + ';color:#000">' + data.flag + '</span>');
    ...
    function bindActivity() {
        ...
        if (el.activityModalClose)
            el.activityModalClose.addEventListener('click', closeActivity);
        if (el.activityModal)
            el.activityModal.addEventListener('click', e => { if (e.target === el.activityModal)
                closeActivity(); });
    ...
    function renderActivityFailed(container) {
        ...
        const nameByUrl = {};
        for (const f of subFeeds)
            if (f.url)
                nameByUrl[f.url] = { name: f.name, region: f.region, hint: f.hint };
        for (const f of customFeeds)
            if (f.url)
                nameByUrl[f.url] = { name: f.name, region: f.scope === 'nation' ? (FeedManager.getNations()[f.nation] || f.nation) : 'Custom', hint: f.subcat };
        container.innerHTML = tracked.map(s => {
            const meta = nameByUrl[s.url] || {};
            const title = meta.name || (() => { try {
    ...
    function renderMyPosts(container) {
        ...
        container.innerHTML = '<div class="activity-list">' + published.map(p => '<div class="activity-item">' +
    ...
    function populateFeedSelects() {
        ...
        if (!scopeSel || !subSel)
            return;
        subSel.innerHTML = FeedManager.subcategories().map(s => '<option value="' + s + '">' + FeedManager.subcatLabel(s, 'global') + '</option>').join('');
        if (nationSel) {
            const nations = FeedManager.getNations();
            nationSel.innerHTML = Object.keys(nations).map(k => '<option value="' + k + '">' + nations[k] + '</option>').join('');
            const wrapper = nationSel.closest('.select-wrapper');
            if (wrapper)
                wrapper.style.display = scopeSel.value === 'nation' ? 'inline-block' : 'none';
        }
        ...
        try {
            ...
            if (el.feedValidateMsg) {
                if (result.valid) {
                    ...
                    if (!el.feedNameInput.value)
                        el.feedNameInput.value = result.title;
                }
                else {
                    validatedFeed = null;
            ...
            validatedFeed = null;
            if (el.feedValidateMsg) {
                el.feedValidateMsg.textContent = 'Validation failed';
                el.feedValidateMsg.className = 'feed-validate-msg error';
            }
        }
    }
    async function handleAddFeed() {
        ...
        if (!name || !url) {
            if (el.feedValidateMsg) {
                el.feedValidateMsg.textContent = 'Enter a name and validate a URL first.';
                el.feedValidateMsg.className = 'feed-validate-msg error';
            }
            return;
        }
    ...
    function renderSubscriptionList() {
        ...
        if (btn)
            btn.addEventListener('click', openSourcesConfigModal);
    }
    ...
    function pickVisibleRegions(allRegions) {
        if (allRegions.length <= VISIBLE_REGION_COUNT + 1)
            return allRegions;
    ...
    function renderSourcesConfigTable() {
        ...
        for (const f of scoped) {
            if (!f.hasRss || !f.url)
                continue;
            if (subsConfigRegion !== 'all' && f.region !== subsConfigRegion)
                continue;
            if (subsConfigStatus === 'refused' && !(window.SourceHealth && SourceHealth.isRefused(f.url)))
                continue;
            if (q) {
        ...
        const scopeTabs = '<div class="scm-scope-tabs">' +
        ...
        const statusFilter = '<div class="scm-status-row">' +
        ...
        const regionSelector = '<div class="scm-regions' + (q ? ' scm-regions-expanded' : '') + '">' +
        ...
        const bulkRow = '<div class="scm-bulk-row">' +
    ...
    function openArticleDetail(link) {
        ...
        if (articleCommentsBtn) {
            const commentCount = (ad.comments && ad.comments.length) || 0;
            if (articleCommentsCount)
                articleCommentsCount.textContent = commentCount;
    ...
    function cleanTitleForTopic(title) {
        ...
        if (!t)
            return '';
        t = t.replace(/^(breaking|just in|update|developing|alert|news|exclusive|latest|watch|video|photos|reported|reports|according to|alert:|developing:)\s*[:\-—]\s*/i, '');
        t = t.replace(/[.!?]+$/, '').trim();
        if (t.length > 0)
    ...
    function compressSentence(s, maxLen) {
        ...
        // Synonym swaps.
        for (const [re, rep] of SYNONYM_TABLE)
        ...
        if (summary) {
            ...
            if (!keyFact) {
                ...
                if (sentences.length > 0)
                    keyFact = compressSentence(sentences[0], 120);
            }
        }
    ...
    function bindArticleClicks() {
        el.main.addEventListener('click', e => {
            ...
            if (likeBtn) {
                ...
                if (ad.like) {
                    ad.like = false;
                    ad.likeCount = Math.max(0, (ad.likeCount || 1) - 1);
                }
                else {
                    ad.like = true;
                    ad.dislike = false;
                    ad.dislikeCount = Math.max(0, (ad.dislikeCount || 0));
                    ad.likeCount = (ad.likeCount || 0) + 1;
                }
                ...
                if (countEl)
                    countEl.textContent = ad.likeCount || 0;
                // Update dislike button if it was active
                const dislikeBtn = likeBtn.parentElement.querySelector('.card-dislike-btn');
                if (dislikeBtn) {
                    dislikeBtn.classList.toggle('active', !!ad.dislike);
                    const dc = dislikeBtn.querySelector('.card-action-count');
                    if (dc)
                        dc.textContent = ad.dislikeCount || 0;
                }
                renderCurrentList();
                return;
            }
            ...
            if (dislikeBtn) {
                ...
                if (ad.dislike) {
                    ad.dislike = false;
                    ad.dislikeCount = Math.max(0, (ad.dislikeCount || 1) - 1);
                }
                else {
                    ad.dislike = true;
                    ad.like = false;
                    ad.likeCount = Math.max(0, (ad.likeCount || 0));
                    ad.dislikeCount = (ad.dislikeCount || 0) + 1;
                }
                saveArticleData(url, ad);
                dislikeBtn.classList.toggle('active', !!ad.dislike);
                ...
                if (countEl)
                    countEl.textContent = ad.dislikeCount || 0;
                // Update like button if it was active
                const likeBtn = dislikeBtn.parentElement.querySelector('.card-like-btn');
                if (likeBtn) {
                    likeBtn.classList.toggle('active', !!ad.like);
                    const lc = likeBtn.querySelector('.card-action-count');
                    if (lc)
                        lc.textContent = ad.likeCount || 0;
                }
                renderCurrentList();
                return;
            }
            ...
            if (cardsBtn) {
                e.stopPropagation();
                if (el.articleModal && el.articleModal.classList.contains('open'))
                    closeArticleModal();
            ...
            if (editBtn) {
                ...
                if (article && article._isPublished && article._pubId)
                    editPublishedArticle(article._pubId);
                return;
            }
        ...
        el.articleModal.addEventListener('click', e => { if (e.target === el.articleModal)
            closeArticleModal(); });
        el.articleModalRead.addEventListener('click', () => {
            const url = el.articleModalRead.dataset.url;
            if (url) {
                closeArticleModal();
                openArticleReader(url, el.articleModalTitle.textContent);
            }
        });
        ...
        el.sourceModal.addEventListener('click', e => { if (e.target === el.sourceModal)
            closeSourceModal(); });
        document.addEventListener('keydown', e => {
            ...
            if (e.key === 'Escape') {
                ...
                if (el.hardRefreshModal && el.hardRefreshModal.classList.contains('open'))
                    closeHardRefreshModal();
                else if (el.sourceModal.classList.contains('open'))
                    closeSourceModal();
                else if (el.commentsPage && el.commentsPage.style.display !== 'none')
                    closeSubView('comments');
                else if (el.articleModal.classList.contains('open'))
                    closeArticleModal();
    ...
    function setTopListStatus(text) {
        ...
        if (ov)
            ov.classList.remove('processing-hidden');
        if (tx)
            tx.textContent = text || 'Loading…';
    }
    function clearTopListStatus() {
        const ov = $('#processing-overlay');
        if (ov)
            ov.classList.add('processing-hidden');
    }
    /* ── Auth ── */
    ...
    function setAuthMode(mode) {
        ...
        if (title)
            title.textContent = mode === 'signin' ? 'Sign In' : 'Sign Up';
        ...
        if (repeatField)
            repeatField.style.display = mode === 'signup' ? 'block' : 'none';
    ...
    function updateAuthUI(user) {
        ...
        if (user) {
            ...
            if (dropdownName)
                dropdownName.textContent = user.user_metadata?.full_name || 'User';
            if (dropdownEmail)
    ...
    function markInputError(id) {
        ...
        if (el)
            el.classList.add('auth-form-input-error');
    }
    ...
    async function signInWithEmail(email, password) {
        ...
        clearInputErrors();
        try {
            const { data, error } = await withTimeout(SupabaseStore.getClient().auth.signInWithPassword({ email, password }), 20000, 'Sign-in');
            if (error) {
    ...
    function handleAuthSubmit() {
        ...
        if (authMode === 'signin') {
            ...
            if (!pwd) {
                showAuthMsg('Please enter a password.', 'error');
                markInputError('auth-password');
                return;
            }
            if (pwd.length < 6) {
            ...
            if (!pwd2) {
                showAuthMsg('Please repeat your password.', 'error');
    ...
    async function handleChangeName() {
        ...
        if (!name || name.length < 2) {
            if (msg) {
                msg.textContent = 'Please enter a name (at least 2 characters).';
        ...
        try {
            const { data, error } = await withTimeout(SupabaseStore.getClient().auth.updateUser({ data: { full_name: name } }), 15000, 'Update name');
            if (error) {
                if (msg) {
    ...
    async function handleChangePassword() {
        ...
        try {
            const { data, error } = await withTimeout(SupabaseStore.getClient().auth.updateUser({ password: newPwd }), 15000, 'Update password');
            if (error) {
                if (msg) {
                    msg.textContent = error.message;
                    msg.classList.add('error');
    ...
    function openChangeAvatarModal() {
        ...
        if (currentUser?.user_metadata?.avatar_url) {
            if (preview) {
                preview.src = currentUser.user_metadata.avatar_url;
    ...
    async function handleAvatarUpload() {
        ...
        try {
            ...
            const { error: metaErr } = await withTimeout(client.auth.updateUser({ data: { avatar_url: publicUrl } }), 15000, 'Update avatar');
            if (metaErr) {
                if (msg) {
                    msg.textContent = metaErr.message;
                    msg.classList.add('error');
                    msg.classList.remove('success');
                }
                return;
    ...
    function openCommentsPage(article) {
        ...
        if (el.commentsPage)
            el.commentsPage.style.display = 'flex';
        if (el.commentsInput) {
            el.commentsInput.value = '';
            el.commentsInput.placeholder = 'Add a comment…';
        }
        if (el.commentsPostBtn)
            el.commentsPostBtn.disabled = true;
        hideReplyPreview();
        renderCommentsList();
    ...
    function closeCommentsPage() {
        if (el.commentsPage)
            el.commentsPage.style.display = 'none';
    ...
    function showReplyPreview(commentText) {
        if (!el.commentsReplyPreview)
            return;
        const txt = commentText.length > 60 ? commentText.slice(0, 60) + '…' : commentText;
        if (el.commentsReplyText)
            el.commentsReplyText.textContent = 'Replying to: ' + txt;
        el.commentsReplyPreview.style.display = 'flex';
        if (el.commentsInput)
            el.commentsInput.placeholder = 'Write a reply…';
    }
    function hideReplyPreview() {
        if (el.commentsReplyPreview)
            el.commentsReplyPreview.style.display = 'none';
        if (el.commentsInput)
            el.commentsInput.placeholder = 'Add a comment…';
    }
    function renderCommentsList() {
        if (!commentsContextArticle || !el.commentsList)
            return;
    ...
    function postComment() {
        ...
        if (countEl)
            countEl.textContent = (ad.comments && ad.comments.length) || 0;
    ...
    function doDeleteComment() {
        if (!commentsPendingDeleteId || !commentsContextArticle)
            return;
        const ad = getArticleData(commentsContextArticle.link);
        if (ad.comments) {
    ...
    function bindAuth() {
        ...
        document.addEventListener('click', e => {
            ...
            if (tab && authModal?.classList.contains('open'))
        ...
        if (avatarBtn)
            avatarBtn.addEventListener('click', e => { e.stopPropagation(); toggleAuthDropdown(); });
        ...
        if (changeNameClose)
            changeNameClose.addEventListener('click', closeChangeNameModal);
        const changeNameModal = $('#change-name-modal');
        if (changeNameModal)
            changeNameModal.addEventListener('click', e => { if (e.target === changeNameModal)
                closeChangeNameModal(); });
        const changeNameForm = $('#change-name-form');
        if (changeNameForm)
            changeNameForm.addEventListener('submit', e => { e.preventDefault(); handleChangeName(); });
        ...
        if (changePwdModal)
            changePwdModal.addEventListener('click', e => { if (e.target === changePwdModal)
                closeChangePasswordModal(); });
        const changePwdForm = $('#change-password-form');
        if (changePwdForm)
            changePwdForm.addEventListener('submit', e => { e.preventDefault(); handleChangePassword(); });
        // Change avatar modal
        const changeAvatarClose = $('#change-avatar-modal-close');
        if (changeAvatarClose)
            changeAvatarClose.addEventListener('click', closeChangeAvatarModal);
        const changeAvatarModalEl = $('#change-avatar-modal');
        if (changeAvatarModalEl)
            changeAvatarModalEl.addEventListener('click', e => { if (e.target === changeAvatarModalEl)
        ...
        if (changeAvatarUpload)
            changeAvatarUpload.addEventListener('click', handleAvatarUpload);
        ...
        if (el.commentsBackBtn)
            el.commentsBackBtn.addEventListener('click', () => closeSubView('comments'));
        ...
        if (el.commentsPostBtn)
            el.commentsPostBtn.addEventListener('click', postComment);
        if (el.commentsInput) {
            el.commentsInput.addEventListener('input', () => {
                if (el.commentsPostBtn)
                    el.commentsPostBtn.disabled = !el.commentsInput.value.trim();
                // Auto-resize
        ...
        if (el.commentsReplyCancel)
            el.commentsReplyCancel.addEventListener('click', () => { commentsReplyToId = null; hideReplyPreview(); });
        if (el.commentsList) {
            el.commentsList.addEventListener('click', e => {
                const replyBtn = e.target.closest('.comment-reply-btn');
                if (replyBtn) {
                    setReplyTo(replyBtn.dataset.commentId);
                    return;
                }
                const deleteBtn = e.target.closest('.comment-delete-btn');
                if (deleteBtn) {
                    confirmDeleteComment(deleteBtn.dataset.commentId);
                    return;
                }
            });
        }
        if (el.deleteCommentConfirm)
            el.deleteCommentConfirm.addEventListener('click', doDeleteComment);
        if (el.deleteCommentCancel)
            el.deleteCommentCancel.addEventListener('click', () => {
                commentsPendingDeleteId = null;
                closeModal('deleteComment');
            });
    ...
    async function init() {
        ...
        bindTopDate();
        bindSourcesConfig();
        if (window.AnalyzeModal)
            AnalyzeModal.bindAll();
        if (window.PublishModal)
            PublishModal.bindAll();
        if (window.FilterModal) {
