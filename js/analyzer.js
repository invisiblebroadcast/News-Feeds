const Analyzer = (() => {
  ...
  function canonicalMetricFromWord(w) {
    ...
    for (const e of METRIC_KEYWORDS) {
      if (e.keys.includes(lw)) return e.metric;
    }
    return null;
  }
  ...
  function detectNumericConflicts(articles, threshold = 0.35) {
    ...
    for (const cluster of clusters) {
      ...
      if (clusterConflicts.length > 0) {
        for (const af of articleFacts) {
          if (out.has(af.article.link)) continue;
    ...
    for (const sent of sentences) {
      ...
      for (const subj of subjects) {
        ...
        for (let i = 0; i < Math.min(6, winTokens.length); i++) {
          ...
          if (/^(is|are|was|were|has|have|had|will|would|could|should|may|might|can|do|does|did|the|a|an|and|or|but|to|of|for|on|in|at|by|with|that|this|these|those|said|says|told|tells|reports?|reported|according)$/.test(w)) continue;
          verbIdx = i;
          break;
        }
        ...
        for (const lb of lookback) {
          if (NEGATION_WORDS.includes(lb)) { negated = true; break; }
        }
  ...
  function detectClaimConflicts(articles, threshold = 0.35) {
    ...
    for (let i = 0; i < articles.length; i++) {
      const root = find(i);
      if (!entityGroups.has(root)) entityGroups.set(root, []);
    ...
    for (const c of vocabClusters) {
      ...
      if (k && !seen.has(k)) { seen.add(k); allClusters.push(c); }
    }
    ...
    for (const cluster of allClusters) {
      ...
      for (const [subjectKey, claimList] of bySubject) {
        ...
        for (let i = 0; i < values.length && !realConflict; i++) {
          for (let j = i + 1; j < values.length && !realConflict; j++) {
            ...
            if (a.verb === b.verb && a.negated !== b.negated) realConflict = true;
            if (!realConflict && isOpposingVerb(a.verb, b.verb)) realConflict = true;
            if (!realConflict && a.verb === b.verb && (a.negated || b['negated'])) realConflict = true;
          }
        }
        if (!realConflict) continue;
      ...
      if (clusterConflicts.length > 0) {
        for (const pac of clusterArticles) {
          if (out.has(pac.article.link)) continue;
  ...
  function formatClaimValue(v) {
    ...
    if (NEGATION_WORDS.indexOf(v.verb) >= 0) return v.verb;
  ...
  function sourceAuthorityWeight(source) {
    ...
    if (SOURCE_AUTHORITY[s] != null) return SOURCE_AUTHORITY[s];
    for (const k of Object.keys(SOURCE_AUTHORITY)) {
      if (s.startsWith(k) || k.startsWith(s)) return SOURCE_AUTHORITY[k];
    }
    return 1.0;
  }
  ...
  function rankByAnalyzer(articles, engagement) {
    ...
    const df = new Map();
    for (let i = 0; i < N; i++) {
      for (const t of termSets[i]) df.set(t, (df.get(t) || 0) + 1);
    }
    ...
    for (let i = 0; i < N; i++) {
      ...
      let sum = 0;
      for (let k = 0; k < Math.min(TOP_N, pairs.length); k++) sum += pairs[k].score;
      tfidfSums[i] = sum;
    }
    ...
    for (let i = 0; i < N; i++) {
      ...
      if (!t || isNaN(t)) { recency[i] = 0.5; continue; }
    ...
    for (let i = 0; i < N; i++) {
      ...
      if (!mySet.size) { buzz[i] = 1; continue; }
      let related = 0;
      for (let j = 0; j < N; j++) {
        if (i === j) continue;
        for (const t of mySet) if (termSets[j].has(t)) { related++; break; }
      }
    ...
    for (let i = 0; i < N; i++) {
      ...
      for (const k in ALARMING_HINTS) {
        if (t.indexOf(k) !== -1) s += ALARMING_HINTS[k];
      }
      alarm[i] = s;
    }
